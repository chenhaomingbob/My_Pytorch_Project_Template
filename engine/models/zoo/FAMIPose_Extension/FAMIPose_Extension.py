...
from engine.layers import conv_bn_relu, ChainOfBasicBlocks
from torchvision.ops.deform_conv import DeformConv2d
from ..backbones import HRNetPlus
import kornia
from utils.utils_constant import TRAIN_PHASE

__all__ = ["FAMIPose_Extension"]
BN_MOMENTUM = 0.1
...
class FAMIPose_Extension(nn.Module):
    ...
    def __init__(self, cfg, is_train=None, **kwargs):
        super(FAMIPose_Extension, self).__init__()
        self.logger = logging.getLogger(__name__)
        self.num_joints = cfg.model.num_joints
        self.pretrained = cfg.model.pretrained

        self.pretrained_layers = ['*']
        self.hrnet = HRNetPlus(cfg)
        ...
        self.dcn_offset_3 = conv_bn_relu(48, n_offset_channel, 3, 1, padding=3, dilation=3, has_bn=False, has_relu=False)
        ...
        self.dcn_offset_4 = conv_bn_relu(48, n_offset_channel, 3, 1, padding=3, dilation=3, has_bn=False, has_relu=False)
    ...
    def forward(self, kf_x, sup_x, **kwargs):
        ...
        for i in range(num_sup):
            sup_bb_hm, sup_bb_feat = sup_bb_hm_list[i], sup_bb_feat_list[i]  # [B,17,96,72] [B,48,96,72]

            feat_offset = self.feat_global_offset_layers(sup_bb_feat - kf_bb_feat)  # [B,2*48]
            feat_offset = feat_offset.reshape(B, 48, 2)

            global_aligned_feat = []
            for num_channel in range(48):
                offset_params = torch.eye(3)[0:2].view(1, 2, 3).repeat(B, 1, 1).to(sup_bb_feat.device)  # [1,2,3]
                offset_params[:, 0, 2], offset_params[:, 1, 2] = feat_offset[:, num_channel, 0], feat_offset[:, num_channel, 1]
                global_aligned_feat_channel = kornia.geometry.warp_affine(sup_bb_feat[:, num_channel:num_channel + 1], offset_params, dsize=(H, W))
                global_aligned_feat.append(global_aligned_feat_channel)
            global_aligned_feat = torch.cat(global_aligned_feat, dim=1)
        ...
        if self.training:
            ...
            return final_hm, [], kf_bb_hm, mi_loss_list, kf_bb_feat, aligned_sup_feat_list
        else:
    ...
    def init_weights(self, *args, **kwargs):
        ...
        if osp.isfile(self.pretrained):
            pretrained_state_dict = torch.load(self.pretrained, map_location="cpu")
    ...
    def feat_label_mi_estimation(self, Feat, Y):
        ...
        mi = kl_div(input=self.softmax(pred_Y.detach() / temperature), target=self.softmax(Y / temperature), reduction='mean')  # pixel-level
