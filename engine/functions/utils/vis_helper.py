...
from datasets.commond import coco2posetrack_ord_infer
from datasets.zoo.posetrack import POSETRACK_joint_pairs, POSETRACK_joint, COLOR_DICT, POSETRACK_joint_name_color


def draw_skeleton_in_origin_image(batch_image_list, batch_joints_list, batch_bbox_list, save_dir, vis_skeleton=True, vis_bbox=True, sure_threshold=0.2):
    """
    ...
    for index, image_path in enumerate(batch_image_list):
        ...
        if osp.exists(vis_image_save_path):
            ...
            processed_image = add_poseTrack_joint_connection_to_image(processed_image, final_coords, sure_threshold=sure_threshold,
...
def draw_jhmdb_skeleton_in_origin_image(batch_image_list, batch_joints_list, batch_bbox_list, save_dir, vis_skeleton=True, vis_bbox=True):
    """
    :param batch_image_list:  batch image path
    :param batch_joints_list:   joints coordinates in image Coordinate reference system
    :batch_bbox_list: xyxy
    ...
    for index, image_path in enumerate(batch_image_list):
        ...
        if osp.exists(vis_image_save_path):
            ...
            processed_image = add_jhmdb_joint_connection_to_image(processed_image, final_coords, sure_threshold=0.2,
                                                                  flag_only_draw_sure=True) if vis_skeleton else processed_image
            processed_image = add_bbox_in_image(processed_image, bbox) if vis_bbox else processed_image
...
def add_poseTrack_joint_connection_to_image(img_demo, joints, sure_threshold=0.8, flag_only_draw_sure=False, ):
    for joint_pair in POSETRACK_joint_pairs:
        ...
        if sure1 > sure_threshold and sure2 > sure_threshold:
            ...
            cv2.line(img_demo, (int(x1), int(y1)), (int(x2), int(y2)), color, thickness=8)
    return img_demo
