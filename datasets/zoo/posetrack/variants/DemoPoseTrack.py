...
from datasets.commond import get_affine_transform, fliplr_joints, exec_affine_transform, \
    generate_heatmaps, half_body_transform, convert_data_to_annorect_struct, build_transforms
...
from .posetrack_utils import video2filenames, evaluate


@DATASET_REGISTRY.register()
class DemoPoseTrack(VideoDataset):
    """
        DemoPoseTrack
    """

    def __init__(self, cfg, phase, **kwargs):
        super(DemoPoseTrack, self).__init__(cfg, phase, **kwargs)
    ...
    def __getitem__(self, key_item_index):
        ...
        if self.model_input_type == "single_ref_frame":
            ...
            input_x, ref_x, target_heatmaps, target_heatmaps_weight, meta = self._get_spatiotemporal_window(data_item)
            neighbor_ref_x = torch.cat(ref_x, dim=0)
    ...
    def _get_single_ref(self, data_item):
        ...
        next_delta_range = range(1, min((num_frames - current_idx) if is_posetrack18 else (num_frames - current_idx + 1), farthest_distance))
        ...
        for delta in delta_list:
            ...
            ref_image_file = osp.join(osp.dirname(image_file_path), str(ref_input_id).zfill(zero_fill) + '.jpg')
    ...
    def evaluate(self, cfg, preds, output_dir, boxes, img_path, *args, **kwargs):
        ...
        AP = evaluate(annot_dir, output_dir, eval_track=False)[0]
